...
# matrix fatorization with no bias
...
def main():
    ...
    model.fit([user_train, movie_train], rating_train, epochs=5,
              validation_data=([user_valid, movie_valid], rating_valid), batch_size=64)
    model.save(index4)
    ...
    with open('./result/mf_nobias_result.csv', 'w') as f:
